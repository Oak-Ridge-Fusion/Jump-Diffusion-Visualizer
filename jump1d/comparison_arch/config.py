...
class Config:
    ...
    # ---- Conditional RealNVP / Neural Spline Flow ------------------------------
    # All capacity/training fields left at the ORIGINAL baseline values (see
    # comparison/config.py) -- coupling_type is the only thing under test here.
    nf_epochs: int = 200
    nf_n_coupling: int = 8
    ...
    nf_coupling_type: str = "spline"   # "affine" (original RealNVP) | "spline" (NSF)
    nf_n_bins: int = 8                 # spline only: number of spline bins per dim
    nf_tail_bound: float = 5.0         # spline only: linear-tail cutoff, in std-normalised units

    # ---- Conditional DDPM -------------------------------------------------------
    # Capacity/epoch/timestep fields left at ORIGINAL baseline values --
    # parameterization + sampler are the only things under test here.
    diff_epochs: int = 200
    ...
    diff_parameterization: str = "v"     # "eps" (Ho et al. 2020) | "v" (Salimans & Ho 2022)
    diff_sampler: str = "ddim"           # "ancestral" (original) | "ddim" (Song et al. 2020)
    diff_ddim_steps: int = 50            # only used when diff_sampler == "ddim"
    diff_ddim_eta: float = 0.0           # 0 = fully deterministic DDIM
