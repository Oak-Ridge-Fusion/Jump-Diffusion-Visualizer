...
models/realnvp.py
==================
Conditional RealNVP normalizing flow for ``p(X_{t+Delta t} | X_t)``.

Architecture (matches the spec): 8 conditional affine coupling layers, each
preceded by an ActNorm and followed by a fixed permutation, with a standard
normal base distribution.  Trained by maximising the exact log-likelihood of
the data under the change-of-variables formula.
...
from models.layers import ActNorm, ConditionalAffineCoupling, Permutation, make_checkerboard_mask
    ...
    def __init__(
        ...
        for i in range(n_coupling):
            ...
            self.couplings.append(
                ConditionalAffineCoupling(dim, context_dim, hidden_dim, n_hidden_layers, mask)
            )
