...
Ground truth = the fine integrator (simulate_bounded_step).
Needs:  <data_dir>/ckpt_flow.pt   (from flow_training.py)
        <data_dir>/ckpt_exit.pt   (from exit_training.py)

Run:  python inference.py [model]   ->  B0_onestep.png, B1_onestep.png, B2_rollout.png, metrics.json
...
def b0_onestep(net, ck, eh, cfg, dev, rng, metrics):
    ...
    for k, x0 in enumerate(starts):
        ...
        # --- free-space jump-count mixture (closed form; only exact for
        #     constant-coefficient "bounded") ---
