...
class Config:
    ...
    n_ref_mc: int = 200000
    rollout_K: int = 0               # big ML steps for the terminal-time test; 0 = auto (4 for "bounded", 8 otherwise)

    # ---- bounded-domain model (model="bounded"): absorbing BCs, RE-aligned ---
    # Positive jump mean -> the one-step density is a visibly MULTIMODAL mixture
    # (components at x + 1.5 n) and there is net jump-driven transport toward the
    # upper wall (RE-like: knock-on collisions kick momentum up).
    x_min: float = 0.0
    x_max: float = 6.0
    drift_b: float = 0.0             # drift inside the domain (0 = Brownian + jumps)
    sigma_b: float = 0.4
    lam_b: float = 1.0               # jump rate
    jump_mean_b: float = 1.5
    jump_std_b: float = 0.3
    exit_hidden: int = 256
    transition_test_x: tuple = (1.0, 5.0)  # fixed starts for the B0 transition and B2 rollout figures

    # ---- 1D STATE-DEPENDENT bounded model (model="bounded_sd") --------------
    # dx = kappa (x* - x) dt + sigma(x) dW + jumps,  sigma(x) = sigma0 + sigma1 x,
    # rate lam(x) = lam0 + lam1 x,  jump sizes N(jmean0 + jmean1 x, jstd^2).
    # The increment law now varies with x, so the flow map's conditioning (and
    # the KNN-smoothing term in the analysis) is actually exercised.
    kappa_sd: float = 0.5
    xstar_sd: float = 2.5
    sigma0_sd: float = 0.3
    sigma1_sd: float = 0.05
    lam0_sd: float = 1.3
    lam1_sd: float = -0.15
    jmean0_sd: float = 1.5
    jmean1_sd: float = -0.08
    jstd_sd: float = 0.3

    device: str = "cuda"
    data_dir: str = "auto"           # auto -> per-model folder (artifacts_bd, artifacts_bsd, ...)

    def __post_init__(self):
        if self.data_dir == "auto":
            self.data_dir = {"merton": "artifacts", "double_well": "artifacts_dw",
                             "bounded": "artifacts_bd",
                             "bounded_sd": "artifacts_bsd"}[self.model]
        if self.rollout_K == 0:
            self.rollout_K = 4 if self.model == "bounded" else 8
    ...
    def to_dict(self):
        ...
        d["transition_test_x"] = list(self.transition_test_x)
        return d
...
def bounded_coeffs(x, cfg: Config):
    """(drift, sigma, lam, jump_mean, jump_std) evaluated at state x, for the
    bounded family:
      model="bounded"    : constant coefficients (multimodal via jump_mean_b),
      model="bounded_sd" : state-dependent drift / sigma / rate / jump law --
                           the case where conditioning on x actually matters."""
    x = np.asarray(x, dtype=np.float64)
    if cfg.model == "bounded_sd":
        return (cfg.kappa_sd * (cfg.xstar_sd - x),
                cfg.sigma0_sd + cfg.sigma1_sd * x,
                cfg.lam0_sd + cfg.lam1_sd * x,
                cfg.jmean0_sd + cfg.jmean1_sd * x,
                np.full_like(x, cfg.jstd_sd))
    return (np.full_like(x, cfg.drift_b), np.full_like(x, cfg.sigma_b),
            np.full_like(x, cfg.lam_b), np.full_like(x, cfg.jump_mean_b),
            np.full_like(x, cfg.jump_std_b))


def simulate_bounded_step(x0, cfg: Config, rng, n_sub=None):
    """One coarse step dt of  dx = b(x) dt + sigma(x) dW + jumps  on the domain
    [x_min, x_max] with ABSORBING boundaries (coefficients from bounded_coeffs;
    the jump rate and jump law are evaluated at the pre-jump state each
    sub-step).  Returns (x_end, alive):
    ...
    for _ in range(n_sub):
        ...
        if not a.any():
            break
        bx, sx, lx, jmx, jsx = bounded_coeffs(x, cfg)
        # jump sub-step (rate and jump law at the pre-jump state)
        fired = a & (rng.random(x.shape) < (1.0 - np.exp(-lx * dt_sub)))
        if fired.any():
            x[fired] += rng.normal(jmx[fired], jsx[fired])
        # diffusion sub-step (only living particles move)
        x[a] += bx[a] * dt_sub + sx[a] * sqrt_dt * rng.standard_normal(int(a.sum()))
...
def bounded_free_logpdf(y, x0, cfg: Config, T=None, n_max=60):
    """Poisson-weighted Gaussian mixture for the UNBOUNDED analog of the bounded
    model (drift_b, sigma_b, lam_b, jumps N(jump_mean_b, jump_std_b^2)) -- i.e.
    the exact transition density if there were no absorbing boundary.  Mid-domain
    the bounded survivor sub-density approaches this; near the walls absorption
    removes mass.  Used as a visual reference for the jump-count mixture.
    Exact only for model="bounded" (constant coefficients)."""
    assert cfg.model == "bounded"
    T = cfg.dt if T is None else T
    y = np.asarray(y, dtype=np.float64)
    lam_dt = cfg.lam_b * T
    comps = np.empty((n_max + 1, y.shape[0]))
    for n in range(n_max + 1):
        mean = x0 + cfg.drift_b * T + n * cfg.jump_mean_b
        var = cfg.sigma_b ** 2 * T + n * cfg.jump_std_b ** 2
        log_pois = -lam_dt + n * math.log(lam_dt) - math.lgamma(n + 1)
        comps[n] = log_pois - 0.5 * np.log(2 * math.pi * var) - 0.5 * (y - mean) ** 2 / var
    m = comps.max(axis=0)
    return m + np.log(np.sum(np.exp(comps - m), axis=0))
...
def bounded_free_components(y, x0, cfg: Config, T=None, n_max=3):
    """The first jump-count components of the free-space mixture: a list of
    (n, poisson_weight, gaussian_pdf(y)) for n = 0..n_max -- the 'n jumps during
    the step' cases the one-step transition density mixes over.  With
    jump_mean_b = 0 all components share the same center (unimodal mixture)."""
    T = cfg.dt if T is None else T
    y = np.asarray(y, dtype=np.float64)
    lam_dt = cfg.lam_b * T
    out = []
    for n in range(n_max + 1):
        mean = x0 + cfg.drift_b * T + n * cfg.jump_mean_b
        var = cfg.sigma_b ** 2 * T + n * cfg.jump_std_b ** 2
        w = math.exp(-lam_dt) * lam_dt ** n / math.factorial(n)
        pdf = np.exp(-0.5 * (y - mean) ** 2 / var) / math.sqrt(2 * math.pi * var)
        out.append((n, w, pdf))
    return out
